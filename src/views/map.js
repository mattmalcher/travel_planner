...
export function renderMap() {
  ...
  for (const s of sortSegments(HD.segments)) {
    if (s.type === 'transport') {
      ...
      L.marker(c, { icon: mpin('#b45309', 'H') }).addTo(HM).bindPopup(`<strong>${s.name}</strong><br>${s.host}<br>${s.address}<br>Check-in: ${fmtDate(s.checkin.date)}`);
    } else if (s.type === 'event' && s.lat) {
      const c = [s.lat, s.lng]; allc.push(c);
      L.marker(c, { icon: mpin('#15803d', 'E') }).addTo(HM).bindPopup(`<strong>${s.name}</strong><br>${s.venue || ''}<br>${fmtDate(s.date)}`);
    }
  }
