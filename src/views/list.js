...
function renderAccom(s) {
  return `<div style="margin-top:8px;font-size:12px;color:var(--color-text-secondary);display:flex;flex-wrap:wrap;gap:8px">
    <span><i class="ti ti-door-enter" style="font-size:12px;vertical-align:-1px" aria-hidden="true"></i> In after ${s.checkin.from} · ${fmtDayLong(s.checkin.date)}</span>
    <span><i class="ti ti-door-exit" style="font-size:12px;vertical-align:-1px" aria-hidden="true"></i> Out by ${s.checkout.by} · ${fmtDayLong(s.checkout.date)}</span>
...
function renderEvent(s) {
  ...
  if (s.pricing) {
    pr = Object.entries(s.pricing).map(([k, v]) => {
      const l = k.replace(/_/g, ' ');
      const val = v.amount !== undefined ? `€${v.amount}` : (v.from !== undefined ? `€${v.from}–€${v.to}` : '');
      return `${l}: ${val}`;
    }).join(' · ');
  }
  return `<div style="margin-top:8px;font-size:12px;color:var(--color-text-secondary);display:flex;flex-wrap:wrap;gap:8px">
